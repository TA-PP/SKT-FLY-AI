...
def print_matInfo(name, image):
    ...
    ## depth, channel 출력
    print('%12s: depth(%s), channel(%s) -> mat_type(%c%d)'
            %(name, image, nchannel, mat_type, nchannel))

title1, title2 = 'gray2gray', 'gray2color'                  # 윈도우 이름
gray2gray = cv2.imread('OpenCV/images/read_gray.jpg', cv2.IMREAD_GRAYSCALE)   # 명암도
gray2color = cv2.imread('OpenCV/images/read_gray.jpg', cv2.IMREAD_COLOR)      # 컬러 영상

## 예외처리 - 영상파일 읽기 여부 조사
if gray2gray is None or gray2color is None:
    raise Exception('영상파일 읽기 에러')

print('행렬 좌표 (100, 100) 화솟값')
